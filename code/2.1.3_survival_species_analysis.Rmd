...
ndvi <- read.csv(here("data", "clean_data", "ndvi_clean.csv")) %>% 
  select(!X) %>% 
  mutate(date = ymd(date),
         month = month(date),
         year = year(date)) %>% 
  select(plot, ndvi, month, year)
...
survival_environmental_chap <- survival_environmental %>%  
  ...
  inner_join(ndvi, by = c("plot", "month", "year")) %>% 
  mutate(ndvi = ndvi*plants_alive,
         ndvi = scale(ndvi))
...
##RHOV
```{r}
cox.zph(coxme(Surv(start_time, end_time, dead) ~ ndvi + soil_moisture + par + (1|plot), 
              data = survival_environmental_rhov))
coxme(Surv(start_time, end_time, dead) ~ ndvi + soil_moisture + par + (1|plot), 
              data = survival_environmental_rhov) %>% 
...
plot(cox.zph(coxme(Surv(start_time, end_time, dead) ~ ndvi + soil_moisture + par + (1|plot), 
              data = survival_environmental_ceol)))
cox.zph(coxme(Surv(start_time, end_time, dead) ~ ndvi + soil_moisture + par + (1|plot), 
              data = survival_environmental_ceol %>% filter(end_time < 141)))
cox.zph(coxme(Surv(start_time, end_time, dead) ~ ndvi + soil_moisture + par + (1|plot), 
              data = survival_environmental_ceol %>% filter(end_time > 141)))
coxme(Surv(start_time, end_time, dead) ~ ndvi + soil_moisture + par + (1|plot), 
              data = survival_environmental_ceol %>% filter(end_time < 141)) %>% tbl_regression(exp = T)
coxme(Surv(start_time, end_time, dead) ~ ndvi + soil_moisture + par + (1|plot), 
              data = survival_environmental_ceol %>% filter(end_time > 141)) %>% tbl_regression(exp = T)
```
