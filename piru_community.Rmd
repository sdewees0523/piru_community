...
survival_summary <- survival_data %>% 
  ...
  mutate(ci_lower = case_when(ci_lower < 0 ~ 0,
                              TRUE ~ ci_lower),
         ci_upper = case_when(ci_upper > 100 ~ 100,
                              TRUE ~ ci_upper))
...
survival_community <- survival_data %>% 
  ...
  group_by(community, plot, month) %>%
    summarise(alive_sum = sum(number_alive),
              planted_sum = sum(number_planted)) %>% 
  ungroup() %>% 
  mutate(percent_alive = (alive_sum/planted_sum)*100) %>% 
...
ggplot(survival_community, aes(x = month, y = mean_survival, col= as.factor(community)), size = 5) +
  geom_point()+
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.3 )+
  theme_classic() +
  theme(text = element_text(size = 10))+
